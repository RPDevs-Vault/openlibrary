...
class ReadProcessor:
    ...
    def get_readitem(self, iaid, orig_iaid, orig_ekey, wkey, status, publish_date):
        ...
        if status == "full access":
            ...
            itemURL = "http://openlibrary.org{}/{}/borrow".format(ekey, helpers.urlsafe(edition.get("title", "untitled")))
        result = {
            # XXX add lastUpdate
            "enumcron": False,
    ...
    def make_record(self, bib_keys):
        ...
        iaids_tosort = [(iaid, getstatus(self, iaid), getdate(self, iaid)) for iaid in iaids]
        ...
        items = [self.get_readitem(iaid, orig_iaid, orig_ekey, wkey, status, date) for iaid, status, date in iaids_tosort]  # if status != 'missing'
        ...
        result = {
            "records": {
                data["key"]: {
                    "isbns": [subitem for sublist in (ids.get("isbn_10", []), ids.get("isbn_13", [])) for subitem in sublist],
                    "issns": [],
                    "lccns": ids.get("lccn", []),
                    "oclcs": ids.get("oclc", []),
